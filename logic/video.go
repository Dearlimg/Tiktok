...
func (videoService *VideoServiceImpl) Feed(latestTime time.Time, userId int64) ([]model.Video, time.Time, error) {
	videos := make([]model.Video, 0, config.VIDEO_NUM_PER_REFRESH)
	plainVideos, err := mysql.GetVideosByLatestTime(latestTime)
	if err != nil {
...
func (videoService *VideoServiceImpl) PublishList(userId int64) ([]model.Video, error) {
	videos := make([]model.Video, 0, config.VIDEO_INIT_NUM_PER_AUTHOR)
	plainVideos, err := mysql.GetVideosByUserId(userId)
	if err != nil {
...
func (videoService *VideoServiceImpl) getRespVideos(videos *[]model.Video, plainVideos *[]mysql.Video, userId int64) error {
...
func (videoService *VideoServiceImpl) combineVideo(video *model.Video, plainVideo *mysql.Video, userId int64) error {
...
func (videoService *VideoServiceImpl) GetVideoListById(videoIdList []int64, userId int64) ([]model.Video, error) {
	videoList := make([]model.Video, 0, config.VIDEO_INIT_NUM_PER_AUTHOR)
	plainVideoList, _ := mysql.GetVideoListById(videoIdList)
...
func (videoService *VideoServiceImpl) GetVideoCnt(userId int64) (int64, error) {
	return mysql.GetVideoCnt(userId)
}

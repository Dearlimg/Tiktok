...
func (commentService *CommentServiceImpl) CommentAction(comment mysql.Comment) (model.Comment, error) {
	csi := GetCommentServiceInstance()
	commentRes, err := mysql.InsertComment(comment)
	if err != nil {
...
func (commentService *CommentServiceImpl) GetCommentList(videoId int64, userId int64) ([]model.Comment, error) {
	...
	plainCommentList, err := mysql.GetCommentList(videoId)
	// 拿评论出错
	if err != nil {
		log.Println(err.Error())
		return nil, err
	...
	commentInfoList := make([]model.Comment, 0, n)
	var wg sync.WaitGroup
	wg.Add(n)
	for _, comment := range plainCommentList {
		var commentData model.Comment
		go func(comment mysql.Comment) {
...
func (commentService *CommentServiceImpl) CombineComment(comment *model.Comment, plainComment *mysql.Comment) error {
